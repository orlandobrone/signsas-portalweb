<?  header('Content-type: text/html; charset=iso-8859-1');
	...
	$query = "SELECT SQL_CALC_FOUND_ROWS * FROM inventario LIMIT $start, $pagesize";
...

		{
...

			{
...

				{
					...
					$query = "SELECT * FROM inventario ORDER BY" . " " . $sortfield . " DESC LIMIT $start, $pagesize";
...

				{
					...
					$query = "SELECT * FROM inventario ORDER BY" . " " . $sortfield . " ASC LIMIT $start, $pagesize";
	...
	while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) { 
		...
		if($row['id'] == 1539):
			$arrayP = $obj->getPromedioTotalGal();
			$row['cantidad'] = $arrayP['total_gal'];
			$row['costo_unidad'] = $arrayP['ponderado'];
		endif;
		...
		$customers[] = array(
			'id' => $row['id'],
			'nombre_material' => utf8_encode($row['nombre_material']),
