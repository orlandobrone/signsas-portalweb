<?php  include "../../restrinccion.php";  ?>
    ...
    $(document).ready(function () {
            ...
            var source = {
                 ...
                 datafields: [
					 ...
					 { name: 'total_facturado', type: 'number'}
                ],
				cache: false,
			    url: 'ajax_data.php',
				sortcolumn: 'id',
            ...
            $("#jqxgrid").jqxGrid(
            {
                ...
                columns: [
				  ...
				  { text: 'Valor Facturado', datafield: 'valor_facturado', filtertype: 'textbox', filtercondition: 'starts_with', cellsformat: 'c2',cellsalign: 'right', aggregates: [{ 'Total':
					  function (aggregatedValue, currentValue, column, record) {
						  totalC = record['total_facturado'];
						  console.log(totalC);
						  if(totalC != '')
						  	$('#valor_total').val(totalC);
						  return totalC;
					  }
                  }], aggregatesrenderer: function (aggregates) {
                      var value = $('#valor_total').val();
                      var renderstring = '<div style="float: right; margin: 4px; overflow: hidden;">' + "Total" + ': $' + addCommas(value) + '</div>';
                      return renderstring;
                  }},	
			...
			$('#btnUpdateHitos').click(function(){
				...
				swal({   
					...
					if(res){
						...
						window.open("ajax_modificar.php?allfields="+allfield);
									
						/*$.ajax({
							url: 'ajax_modificar.php',
							dataType: 'json', // Notice! JSONP <-- P (lowercase)
							data: { allfields: allfield},
							type: 'post',
			...
			$('#btnCheckHitos').click(function(){
				window.open("verficar_datos.php");
			});
			
			
			$('#downFormato').click(function(){
				window.open("carge.xls");
			});
    ...
    <div id="cuerpo">
            ...
            <div style="background:#ECECEC; margin-bottom:15px;border:1px solid #CCC;padding:10px;margin-top:5px;border-radius:5px;-moz-border-radius:5px;-webkit-border-radius:5px; width:96%; margin-top:20px;">
             ...
             <input value="Bajar Formato" id="downFormato" type="button" class="btn_table" />
             
             <input value="Verificar Datos" id="btnCheckHitos" type="button" class="btn_table" />
             
             <? if(in_array(373,$_SESSION['permisos'])): ?>
             <input value="Aplicar Script" id="btnUpdateHitos" type="button" class="btn_table" />
             <? endif; ?>
             
             <? if(in_array(372,$_SESSION['permisos'])): ?>
             <input value="Limpiar Tabla" id="btnClearHitos" type="button" class="btn_table" />
             <? endif; ?>
             
             <label>Validar todos los campos</label>
             <input type="checkbox" name="allfields" id="allfields">
          ...
          <!--   <input type="button" value="Exportar a Excel" id='excelExport' class="btn_table" />
            ...
            <input type="hidden" id="valor_total" value="0">
