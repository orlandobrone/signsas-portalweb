<? header('Content-type: text/html; charset=iso-8859-1');
	...
	$query = "SELECT SQL_CALC_FOUND_ROWS * FROM ingreso_mercancia WHERE parent = 1 LIMIT $start, $pagesize";
...

	{
...

		{
			...
			$result = mysql_query($query) or die("SQL Error 1: " . mysql_error().'SQL:'.$query);

			$sql = "SELECT FOUND_ROWS() AS `found_rows`;";

			$rows = mysql_query($query) or die("SQL Error 1: " . mysql_error().'SQL:'.$query);
...

		{
...

			{
...

				{
					...
					$query = "SELECT * FROM ingreso_mercancia WHERE parent = 1 ORDER BY" . " " . $sortfield . " DESC LIMIT $start, $pagesize";
...

				{
					...
					$query = "SELECT * FROM ingreso_mercancia WHERE parent = 1 ORDER BY" . " " . $sortfield . " ASC LIMIT $start, $pagesize";
	...
	while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
		...
		$sqli = mysql_query("SELECT COUNT(*) AS totalitems FROM ingreso_mercancia 
							WHERE id_ingreso =".$row['id']) or die(mysql_error().'SQL:'.$sqli);
