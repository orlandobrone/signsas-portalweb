<?  header('Content-type: text/html; charset=iso-8859-1');
	...
	$query = "SELECT SQL_CALC_FOUND_ROWS *, s.id AS ID
			  FROM anticipo AS s 
			  WHERE s.publicado != 'draft' AND s.estado = 1 AND s.id_centroscostos = 4 AND s.prioridad NOT IN ('REINTEGRO','GIRADO')
...

		{
...

		    {
...

				{
					...
					$where .= ") AND s.id_centroscostos = 4 AND s.prioridad NOT IN ('REINTEGRO','GIRADO')".$orden_trabajo;
			...
			if($hito == true):
				...
				$where = ' WHERE s.estado = 1 '.$orden_trabajo;
...

			{
...

				{
					...
					$query = "SELECT *, s.id AS ID, c.nombre AS centro_costo, c.codigo AS codigo  
							  ...
							  WHERE ".$filtroRegion." s.publicado != 'draft' AND s.estado = 1 AND s.prioridad NOT IN ('REINTEGRO','GIRADO') AND s.id_centroscostos = 4
...

				{
					...
					$query = "SELECT *, s.id AS ID, c.nombre AS centro_costo, c.codigo AS codigo 
			  				  FROM anticipo AS s 
	          				  LEFT JOIN linea_negocio AS c ON  s.id_centroscostos = c.id 
							  WHERE ".$filtroRegion." s.publicado != 'draft'  AND s.estado = 1 AND s.prioridad NOT IN ('REINTEGRO','GIRADO') AND id_centroscostos = 4
