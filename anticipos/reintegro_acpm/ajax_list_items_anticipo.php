<?php
	...
	$query = "	SELECT i.*, i.id AS ID, h.estado AS estado_hito, h.nombre AS nombre_hito
				
				FROM items_anticipo AS i

				LEFT JOIN hitos AS h ON  h.id = i.id_hitos

			  	WHERE i.id_anticipo = ".$_GET['id']." AND i.estado = 1 AND i.cant_galones > 0 LIMIT $start, $pagesize";
...

		{
...

		    {
...

				{
					...
					$where .= ") AND i.estado = 1 AND i.cant_galones > 0";
...

			{
...

				{
					...
					$query = "SELECT *, i.id AS ID, i.id_hitos AS idHitos, i.estado AS estado_anti, h.estado AS estado_hito
							  FROM items_anticipo AS i
							  LEFT JOIN hitos AS h ON  h.id = i.id_hitos 
							  WHERE i.id_anticipo = ".$_GET['id']." AND i.estado = 1 AND i.cant_galones > 0
	...
	while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
		...
		$total_hito = $row['valor_galon'] * $row['cant_galones'];
		
		$totalGal = $row['cant_galones'] - $obj->getTotalGalReintByHito($row['id_hitos']);
		...
		$customers[] = array( 
			...
			'cant_galones'=>$totalGal, 
			'cant_galones_r'=>$obj->getTotalGalReintByHito($row['id_hitos']),
			'total_hito'=>$total_hito,
			'reintegro_galones'=>0
		  );
