<?  header('Content-type: text/html; charset=iso-8859-1');
	...
	$sql = sprintf("INSERT INTO `anticipo` (`id`, `fecha`, `prioridad`, `nombre_responsable`, `cedula_responsable`, `id_regional`, `id_centroscostos`, `giro`, `total_anticipo`, `banco`, `tipo_cuenta`, `num_cuenta`, `cedula_consignar`, `beneficiario`, `observaciones`, `id_ordentrabajo`, `estado`, `fecha_edit`, publicado) VALUES (NULL, NOW(), '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', 'draft');");
...
<div id="content_form">
    ...
    <h1 style="float:left; margin-left:20px;line-height: 43px;">FORMATO DE SOLICITUD DE ANTICIPO - REINTEGRO</h1> 
...
<form action="javascript: fn_modificar();" method="post" id="frm_per">
	...
	<table style="display:block;">
      ...
      <table style="display:block">      
            ...
            <tr>           
                ...
                <td>Nombre:</td>
                <td>
                 	<input type="text" name="nombre_responsable" id="nombre_responsable" size="40" value="<?=$rs_per['nombre_responsable']?>" class="required" readonly/>
                ...
                <td>Cedula:</td>
                <td>
                 <input type="text" name="cedula_responsable" id="cedula_responsable" size="40" value="<?=$rs_per['cedula_responsable']?>" class="required" readonly/>                
                </td>            
            </tr>
    ...
    <table style="width:100%;">
        <tbody>            
            ...
            <tr>
            	<td>CEDULA:</td>
                <td>
              	<input type="text" name="cedula_consignar" id="cedula_consignar" size="30" alt="integer" class="required" value="<?=$rs_per['cedula_consignar']?>" readonly/>
                </td>  
    ...
    <input type="hidden" name="totalanticipo" id="total_anticipo" class="total_anticipo" value="0"/>   
</form>
  
<form action="javascript: fn_agregar_item();" method="post" id="frm_item">

	<input type="hidden" id="id" name="id" value="<?=$id_anticipo?>" />
    <input type="hidden" value="1" name="reintegro"/>
    ...
    <table style="width:100%">
        <tbody>          	
            <tr>
              ...
              <td>
                  <input type="text" name="total_anticipo" id="total_anticipo"  readonly="readonly" value="0" class="total_anticipo"/>               
              </td>           
        	</tr>       
        </tbody>   
   </table>
   ...
   <table style="width:100%;table-layout: fixed;word-wrap: break-word;">
        <tbody>       
          	...
          	<tr>
                  ...
                  <td colspan="3">
                  	  <? 
					  	if(empty($_GET['valor_fa'])):
					  		$sql = "SELECT  DISTINCT(i.id_hito), h.* FROM items AS i
									LEFT JOIN hitos AS h ON h.id = i.id_hito 
									WHERE i.estado = 0 AND i.id_legalizacion = ".$id_legalizacion;
						else:
							$sql = "SELECT * FROM hitos WHERE estado IN ('PENDIENTE','EN EJECUCION','ADMIN','COTIZADO','EJECUTADO','INFORME ENVIADO') AND id_proyecto = ".$rs_per['id_ordentrabajo'];
					  	endif;
					  ?>
...
<div style="background:#ECECEC; margin-bottom:15px;border:1px solid #CCC;padding:10px;margin-top:5px;border-radius:5px;-moz-border-radius:5px;-webkit-border-radius:5px; width:96%; margin-top:20px;">
       <input name="update" type="button" id="update" value="Reintegrar" class="btn_table" /> 
...
function fn_modificar(){
	...
	var entrar = true;
	<? if(!empty($_GET['valor_fa'])): ?>
		var valor_fa = <?=(!empty($_GET['valor_fa']))?$_GET['valor_fa']:0?>;
		var total_anticipo = $("#total_anticipo").val();
		var res = total_anticipo.split(".00");
		total_anticipo = res[0].replace(",", "");
		
		console.log(parseInt(total_anticipo)+'<'+parseInt(valor_fa));
		
		if(parseInt(total_anticipo) < parseInt(valor_fa)){
			swal("Ups", "Usuario: el valor de reintegro debe ser igual al total del valor de fondo valor de $<?=$_GET['valor_fa']?>", "warning");
			entrar = false;
		}else{
			entrar = true;
		}
	...
	if(entrar){
		...
		$.ajax({
			url: 'ajax_vincular_anticipo_reintegro.php',
			data: str,
			type: 'post',
			success: function(data){
				console.log('entro a modificar el vinculado')
				if(data != "") { 
					alert(data);
				}else{
					fn_cerrar();	
...
function fn_eliminar_item(ide_per, idanticipo){
	...
	var respuesta = confirm("Desea eliminar el item selecionado del anticipo - Reintegro?");
