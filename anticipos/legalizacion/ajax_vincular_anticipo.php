<?
	...
	$obj = new TaskCurrent;	
	//obtiene le total del los items de un anticipo
	$_POST['totalanticipo'] = $obj->getSumaTotalItemsByAnticipo((int)$_POST['id']);
	...
	if(!mysql_query($sql)){
		...
		exit;
			...
			$mensaje = '';
			$para = "ricardo.hernandez@signsas.com";
			//$para = "ingsistemas.ordonez@gmail.com"; 
			// subject
			$titulo = 'Nuevo Anticipo Vinculado #'.(int)$_POST['id'];
			...
			$resultado = mysql_query("SELECT * FROM items_anticipo WHERE estado = 2 AND id_anticipo =".(int)$_POST['id']) or die(mysql_error());
			...
			$total_anticipo = $total_acpm + $total_toes + $total_anticipo + $giro + $viaticos + $mular;		
