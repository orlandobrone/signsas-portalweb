...
?>
	...
	<table rules="all" border="1">
    	...
    	<tr bgcolor="#CCCCCC">
            ...
            <td align="center"><strong>ID Anticpo</strong></td>
...
<?php	
	...
	while ($row_legalizacion = mysql_fetch_array($result, MYSQL_ASSOC)):
		...
		if($row_legalizacion['id'] != ''):
			...
			$resultado_anticipo = mysql_query("SELECT * FROM anticipo WHERE id_legalizacion =".$row_legalizacion['id']) or die(mysql_error());
		...
		$sql = "SELECT * FROM items WHERE id_legalizacion =".$row_legalizacion['id'];
		...
		if($total_items >= 1):
			...
			while ($row_item = mysql_fetch_array($resultado_items, MYSQL_ASSOC)):
					...
					$resultado = mysql_query("SELECT pagado FROM items WHERE id_legalizacion =".$row_item['id_legalizacion']) or die(mysql_error());
					...
					$sql4 = "SELECT o.orden_trabajo AS ot, h.nombre AS nombre_hito, h.id as id_de_hito FROM  orden_trabajo AS o
