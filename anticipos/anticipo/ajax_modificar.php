<?
	...
	if($_POST['action'] != 'edit'):	
		if(empty($_POST['tipo_persona'])):
			echo "Seleccione un tipo de persona antes de continuar";
			exit;
		endif;
	endif;
	
	if($_POST['os_vincular'] == 0){
		...
		if(	   empty($_POST['fecha']) 
			...
			|| empty($_POST['cedula_responsable']) 
			|| empty($_POST['centros_costos'])
			|| empty($_POST['os_vincular'])
			|| empty($_POST['id'])
			|| empty($_POST['regional'])
			|| empty($_POST['orden_trabajo'])
	
		){
			echo "Usted no a llenado todos los campos OS";
			exit;
		}
	}
	...
	if(!mysql_query($sql)){
		...
		$total_anticipo = $obj->getSumaTotalItemsByAnticipo((int)$_POST['id']) + $giro;
