...
<form action="javascript: fn_modificar();" method="post" id="frm_per">
	...
	<input type="hidden" name="total_anticipo" id="total_anticipo" class="total_anticipo"/>
            ...
            <tr>
                ...
                <td>
                 ...
                 <? 
					...
					$rsRegional = mysql_fetch_array($qrPry);
				
					
					if($rsRegional['id_regional'] == 26): 
						...
						$regional = explode(',',$rsRegional['id_regional']);
						...
						if(count($regional) > 1):
							...
							$query2 = " id=".$rsRegional['id_regional'];
						endif;
    ...
    <table>
    	 <tr>
           		<td>Transferencia</td> 
                <td><input type="radio" class="typeInv" name="opcionivn" value="d" checked></td>
                <td>Inventario ACPM</td> 
                <td><input type="radio" class="typeInv" name="opcionivn" value="i"></td>
         </tr>
    </table>
    ...
    <table>
             ...
             <tr>
             	<td>Contratista:</td>
                <td><input type="radio" value="contratista" name="tipo_persona" class="tipo_persona"></td>
                <td>Interno:</td>
                <td><input type="radio" value="interno" name="tipo_persona" class="tipo_persona">
                </td>             	
             </tr>
     </table>       
            
     <table class="os_content" style="display:none;">
        <tr>
            <td style="width:220px;">Ordenes de Servicios:</td>
            <td>
                <? 	if($rsRegional['id_regional'] == 26): 
						$sqlPry = "SELECT * 
								   FROM orden_servicio
								   WHERE aprobado = 1 ORDER BY id DESC"; 
				    else:
						...
						if(count($regional) > 1):
							...
							$query2 = $rsRegional['id_regional'];
						endif;
						
						$sqlPry = "SELECT * 
								   FROM orden_servicio
								   WHERE aprobado = 1 AND id_regional IN(".$query2.") 
								   ORDER BY id DESC";
                ...
                <select id="os_vincular" name="os_vincular"  class="chosen-select required">
    
                    <option value="0">Seleccionar..</option>
    
                    <? while ($rsPry = mysql_fetch_array($qrPry)) { ?>
    
                        <option value="<?=$rsPry['id']?>" 
                                nombre="<?=$rsPry['nombre_contratista']?>"
                                cedula="<?=$rsPry['cedula_contratista']?>"
                                telefono="<?=$rsPry['telefono_contratista']?>"
                                direccion="<?=$rsPry['direccion_contratista']?>"
                                
                                contacto="<?=$rsPry['contacto_contratista']?>"
                                regimen="<?=$rsPry['regimen_contratista']?>"
                                correo="<?=$rsPry['correo_contratista']?>"
                                poliza="<?=$rsPry['poliza_contratista']?>"
                                banco="<?=$rsPry['banco_contratista']?>"
                                tipocuenta="<?=$rsPry['tipocuenta_contratista']?>"
                                numcuenta="<?=$rsPry['numcuenta_contratista']?>"
                                observaciones="<?=$rsPry['observaciones_contratista']?>"
                                contrato="<?=$rsPry['num_contrato_contratista']?>"><?=$rsPry['id']?></option>
                    <? } ?>
        ...
        <tr>
            ...
            <td>
                <input type="text" id="nombre" size="30" disabled/>       
            </td> 
            <td>CEDULA:</td>
            <td>
               <input type="text" id="cedula" size="30" disabled/>  
            </td> 
            <td>TEL&Eacute;FONO:</td>
            <td>
                <input type="text" id="telefono" size="30" disabled/>  
            </td> 
        <tr/>
        <tr>
            <td>DIRECCI&Oacute;N:</td>
            <td>
                <input type="text" id="direccion" size="30" disabled/>  
            </td> 
            <td>CONTACTO:</td>
            <td>
                <input type="text" id="contacto" size="30" disabled/>  
            </td> 
            <td>REGIMEN:</td>
            <td>
                <input type="text" id="regimen" size="30" disabled/>  
            </td> 
        <tr/>
        <tr>
            <td>CORREO:</td>
            <td>
                <input type="text" id="correo" size="30" disabled/>  
            </td> 
            <td>POLIZA:</td>
            <td>
                <input type="text" id="poliza" size="30" disabled/>  
            </td> 
            <td>BANCO:</td>
            <td>
                <input type="text" id="banco" size="30" disabled/>  
            </td> 
        <tr/>
        <tr>
            <td>TIPO CUENTA:</td>
            <td>
                <input type="text" id="tipocuenta" size="30" disabled/>  
            </td> 
            <td>NO. CUENTA:</td>
            <td>
                <input type="text" id="numcuenta" size="30" disabled/>  
            </td> 
            <td>SALDO ACUMULADO:</td>
            <td>
                <input type="text" id="observaciones" size="30" disabled/>  
            </td> 
        <tr/>
        <tr>
            <td>CONTRATO:</td>
            <td>
                <input type="text" id="contrato" size="30" disabled/>  
            </td>
        <tr/>   
     </table>
             
             
     <table class="interno_content" style="display:none;">
     	<tr>        
            <td style="width:220px;">CEDULA:</td>
            <td>   
                <? /*	$sqlPry = "SELECT * FROM beneficiarios 
							   WHERE tipo_persona = 'beneficiario' ORDER BY identificacion ASC"; 
                	$qrPry = mysql_query($sqlPry);*/
                ?>
                
                <select id="cedula_consignar" name="cedula_consignar"  class="cedula_consignar chosen-select">
            ...
            <td>BENEFICIARIO:</td>
            <td>
                <input type="text" name="beneficiario" id="beneficiario_i" size="30" class=""/>       
            </td>       
        ...
        <tr>
            ...
            <td>
                ...
                <input type="text" name="banco" id="banco_i" size="30"  class="" />               
            ...
            <td>
                ...
                <input type="text" name="tipo_cuenta" id="tipo_cuenta_i" size="30" class=""/>       
        ...
        <tr>    
            ...
            <td>
                ...
                <input type="text" name="num_cuenta" id="num_cuenta_i" size="30" class=""/>       
            ...
            <td>
                ...
                <input type="text" name="observaciones" id="observaciones_i" size="30" value="<?=$rs_per['observaciones']?>"/>       
        ...
        <tr>
            <td>TIPO BANCO</td>
            <td colspan="3">
                <input type="radio" name="opcionbanco" value="Bancolombia" class=""/> Bancolombia.                   
                <input type="radio" name="opcionbanco" value="Otros Bancos" class=""/> Otros Bancos.
                <input type="radio" name="opcionbanco" value="Giro Efectivo" class="giro_check"/> Giro Efectivo.
            </td>
        </tr>      
      </table> 
...
<form action="javascript: fn_agregar_item();" method="post" id="frm_item" style="clear:both;">
    ...
    <table style="width:100%">
        ...
        <tbody>          	
            ...
            <tr>
              ...
              <td>
              	 <input type="text" name="total_anticipo" id="total_anticipo" class="total_anticipo"  style="background:#CCC" readonly/>                 
              </td>           
   ...
   <table style="width:100%;">
        ...
        <tbody> 
           ...
           <tr class="invacpm">
                 <td>Galones:<input type="text" name="galones" id="galones2" value="0" alt="integer" class="reset required valor_total_galon" style="width:110px" readonly/>
                 </td>
                 <td>Total:<input type="text" name="acpm2" id="acpm2" value="0" alt="decimal" class="required reset" style="width:120px" readonly />               
                 </td>
           </tr>
...
$(document).ready(function () {
           ...
           /* $('#clearfilteringbutton').click(function () {
			...
			$(".typeInv").click(function(){	
				...
				getloadbeneficiarios();
			});
			
			//obtiene la lista de beneficiarios
			var getloadbeneficiarios = function(){
				
				loaderSpinner();
				
				var tipopersona = $('input[name=tipo_persona]:checked', '#frm_per').val();	
				var tipo = $(".typeInv:radio[name='opcionivn']:checked").val();			
				
				$.getJSON('ajax_list_beneficiarios.php', { tipo_item:tipo, tipo_persona: tipopersona}, 
					...
					function (data) {
					  ...
					  $('.cedula_consignar').removeAttr('disabled');				
					  
					  if(data.length > 0){
						  $.each(data, function (i, v) { 
						  	  
							  var estado = false;
							  if(v.estado == 1 || v.clinton == 1 || v.sgss == 1 || v.tipo_trabajo == 1)
							  	estado = true;							 
			...
			//change del tipo de persona que selecciona
			$('.tipo_persona').change(function(){
				
				var tipo_persona = $('input[name=tipo_persona]:checked', '#frm_per').val();
				$('#id_os').val('');
				$('#os_vincular').val(0);
				
				$('.os_content input[type="text"]').each(function(){
					$(this).val('');
				});
				
				$('.interno_content input[type="text"]').each(function(){
					$(this).val('');
				});
				...
				if(tipo_persona == 'interno'){
					getloadbeneficiarios();
					$('.os_content').hide();
					$('.interno_content').show();
				}else{					
					$('.interno_content').hide();
					$('.os_content').show();					
				}
			});
...
$(document).ready(function(){
	...
	$('#os_vincular').change(function(){
	
		
		$( "#os_vincular option:selected" ).each(function() {
			...
			$('#regimen').val($( this ).attr('regimen') );
			$('#poliza').val($( this ).attr('poliza') );
			$('#banco').val($( this ).attr('banco') );
			$('#tipocuenta').val($( this ).attr('tipocuenta') );
			$('#numcuenta').val($( this ).attr('numcuenta') );
			$('#observaciones').val($( this ).attr('observaciones') );
			$('#contrato').val($( this ).attr('contrato') );
		});
	});
	...
	$('.cedula_consignar').change(function(){
		...
		$( ".cedula_consignar option:selected" ).each(function() {	
			...
			$('#beneficiario_i').val($( this ).attr('nombre'));
			$('#banco_i').val($( this ).attr('entidad'));
			$('#tipo_cuenta_i').val($( this ).attr('tipocuenta'));
			$('#num_cuenta_i').val($( this ).attr('numcuenta'));
			$('#id_os').val(0);
		});
	});
