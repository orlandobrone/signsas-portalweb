<?  header('Content-type: text/html; charset=iso-8859-1');
	...
	//filtrar solamente para su region 
	...
	while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
		...
		if($row['orden_servicio_id'] != 0):
			$sql2 = "SELECT nombre_contratista,cedula_contratista,banco_contratista FROM `orden_servicio` WHERE id = ".$row['orden_servicio_id']; 
			$per2 = mysql_query($sql2);
			$rs_per2 = mysql_fetch_array($per2);			
			$row['beneficiario'] = $rs_per2['nombre_contratista'];
			$row['cedula_consignar'] = $rs_per2['cedula_contratista'];
			$row['banco'] = $rs_per2['banco_contratista'];
		endif;
