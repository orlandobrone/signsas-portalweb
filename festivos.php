...

{
		...
		public function esHabil($fecha){
			...
			$tok = strtok("-");
			$dia = (int)$tok; 
			if(date('N', strtotime($fecha)) < 6 && !$this->esFestivo($dia,$mes))
				return true;
			else 
				return false;
		} 
		
		public function esHabilSabado($fecha){
			$tok = strtok($fecha, "-");
			$anho = (int)$tok;
			$tok = strtok("-");
			$mes = (int)$tok;
			$tok = strtok("-");
			$dia = (int)$tok; 
			if(date('N', strtotime($fecha)) < 7 && !$this->esFestivo($dia,$mes))
				return true;
			else 
				return false;
		} 
		
		public function proxHabil($fecha){
			while(1){
