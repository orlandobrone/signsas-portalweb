<? header('Content-type: text/html; charset=iso-8859-1');
	...
	$query = "SELECT SQL_CALC_FOUND_ROWS * FROM proyecto_costos LIMIT $start, $pagesize";
...

	{
...

		{
			...
			$query = "SELECT * FROM proyecto_costos ".$where;
			...
			$query = "SELECT * FROM proyecto_costos ".$where." LIMIT $start, $pagesize";		
...

		{
...

			{
...

				{
					...
					$query = "SELECT * FROM proyecto_costos ORDER BY" . " " . $sortfield . " DESC LIMIT $start, $pagesize";
...

				{
					...
					$query = "SELECT * FROM proyecto_costos ORDER BY" . " " . $sortfield . " ASC LIMIT $start, $pagesize";
	...
	while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
		...
		$resultado = mysql_query("SELECT nombre FROM proveedor WHERE id =".$row['id_proveedor']) or die(mysql_error());
		...
		$resultado = mysql_query("SELECT nombre FROM tecnico WHERE id =".$row['id_tecnico']) or die(mysql_error());
		...
		$customers[] = array(
			...
			'id_tecnico' => utf8_encode($tecnico),

			'concepto' => utf8_encode($arrConcepto[$row['concepto']]),
