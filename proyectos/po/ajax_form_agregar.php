<?  header('Content-type: text/html; charset=iso-8859-1');
	...
	$sql = sprintf("INSERT INTO `po` (id, estado) VALUES (NUll, 'draft');");

	if(!mysql_query($sql)){
		echo "Error al insertar un nuevo P.O:\n$sql"; 
		exit;
	}
	
	$id_po = mysql_insert_id();
...
<table>
        <tbody>
        	<tr>
                <td>Cliente:</td>
                <td>
                <? $sqlMat = sprintf("SELECT id,nombre FROM cliente ORDER BY id DESC");
                   ...
                   <select class="chosen-select" name="id_cliente" id="id_cliente">
                        <option value="">Seleccione...</option>
                         <? while ($rs_per_mat = mysql_fetch_assoc($perMat)) { ?>
                            <option value="<?=$rs_per_mat['id']; ?>"><?php echo $rs_per_mat['nombre']; ?></option>
                         <? } ?>
                    </select>
                </td>
            </tr>        
        	<tr>        
                <td>No:</td>
                <td>
                 <input type="text" name="no" id="no" size="40"  class="required"/>                
                </td>      
        	 <tr>
                <td>Fecha Inicio:</td>
                <td><input name="fecha_inicio" type="text" id="fecha_inicio" readonly="readonly required" />
          ...
          <tr>
                <td>Fecha Final:</td>
                <td><input name="fecha_final" type="text" id="fecha_final" readonly="readonly required" />
            ...
            <tr>        
                ...
                <td>
                 <input type="text" name="valor_mantenimiento" id="valor_mantenimiento" value="0" size="40" class="money"/>                
                </td>
           </tr>
           <tr>     
                <td>Valor Suministro:</td>
                <td>
                 <input type="text" name="valor_suministro" id="valor_suministro" value="0" size="40" class="money"/>                
                </td> 
           </tr>
           <tr>     
                <td>Valor Total:</td>
                <td>
                 <input type="text" name="valor_total" id="valor_total" value="0" size="40" class="" readonly="readonly"/>                
                </td> 
           </tr>         
...
$(document).ready(function () {
			...
			$('#id_cliente').change(function(){				
				$.getJSON('/proyectos/po/ajax_list_cliente_proyecto.php',{id_cliente:$(this).val()},function(data){
						var options = $('#id_proyecto');
						$('#id_proyecto').empty();						
						$('#id_proyecto').append('<option>Seleccione..</option>');
						...
						$("#id_proyecto").trigger("chosen:updated");
				});
			});
