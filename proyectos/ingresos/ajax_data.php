<? header('Content-type: text/html; charset=iso-8859-1');
	...
	$query = "SELECT SQL_CALC_FOUND_ROWS * FROM ingresos WHERE estado != 'draft' LIMIT $start, $pagesize";
	...
	if (isset($_GET['sortdatafield']))
	{
		...
		if ($sortorder != '')
		{
			if ($_GET['filterscount'] == 0)
			{
				if ($sortorder == "desc")
				{
					$query = "SELECT * FROM ingresos WHERE estado != 'draft' ORDER BY" . " " . $sortfield . " DESC LIMIT $start, $pagesize";
				}
				else if ($sortorder == "asc")
				{
					$query = "SELECT * FROM ingresos WHERE estado != 'draft' ORDER BY" . " " . $sortfield . " ASC LIMIT $start, $pagesize";
				}
			}
			else
			{
	...
	while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
		...
		$resultado = mysql_query(" SELECT nombre 
								   FROM proyectos
								   WHERE id =".$row['id_proyecto']) or die(mysql_error());
		$rows = mysql_fetch_assoc($resultado);
		
		
		$resultado2 = mysql_query("SELECT no 
								   FROM po
								   WHERE id =".$row['id_po']) or die(mysql_error());
		$rows2 = mysql_fetch_assoc($resultado2);

		//<a href="javascript: fn_mostrar_frm_modificar('.$row['id'].');"><img src="../extras/ico/page_edit.png" /></a>&nbsp;
		$customers[] = array(
			'id' => $row['id'],
			'id_ordentrabajo' => $rows['nombre'],
			'id_po' => $rows2['no'],
			'gr' => $row['gr'],			
