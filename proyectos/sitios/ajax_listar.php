<? header('Content-type: text/html; charset=iso-8859-1');
	...
	if (isset($_GET['criterio_usu_per']))
		$sql .= " where ciudad like '%".fn_filtro(substr($_GET['criterio_usu_per'], 0, 16))."%'";
...
<table id="grilla" class="lista" width="690px">
  <thead>
    ...
    ?>
        <tr id="tr_<?=$rs_per['id']?>">
        	<td><?=$rs_per['id']?></td>
            <td><?=($rs_per['regional'])?></td>
            <td><?=($rs_per['departamento'])?></td>           
            <td><?=($rs_per['ciudad'])?></td>
            <td><?=($rs_per['nombre_rb'])?></td>          
            <td><?=$rs_per['direccion']?></td>
            <td><?=$rs_per['tipo_rb']?></td>
